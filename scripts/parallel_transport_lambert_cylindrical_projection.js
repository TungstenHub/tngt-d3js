...
var drag_handler = d3.drag()
	...
	.on("drag", function(d) {
		...
		for (i = 0; i < prec; i++) {
			vy = prev.y+(i+0.5)*dif_y; // http://www2.math.uu.se/~svante/papers/sjN15.pdf
			for (k of Array.from({length: 12}, (x,i) => i)) {
				delta_x = -vy/(1-vy*vy)*dif_x*vectors[k].y-vy/(1-vy*vy)*dif_y*vectors[k].x;// G_11^1*dif_x*vec.x+G_12^1*dif_x*vec.y+G_21^1*dif_y*vec.x+G_22^1*dif_y*vec.y
				delta_y = vy*(1-vy*vy)*dif_x*vectors[k].x+vy/(1-vy*vy)*dif_y*vectors[k].y;// G_11^2*dif_x*vec.x+G_12^2*dif_x*vec.y+G_21^2*dif_y*vec.x+G_22^2*dif_y*vec.y
				vectors[k].x -= delta_x;
				vectors[k].y -= delta_y;
			}
		}
